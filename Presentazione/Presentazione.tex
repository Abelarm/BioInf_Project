...
\begin{itemize}
	\item Ad un'insieme di geni vengono associati tutte le malettie e tutti gli agenti chimici.
...
INSIdEnano è stato implemetato usando $R$ per il back end e Javascript per il front end. Per far comunicare i due linguaggi, sono state usate le librerie HTMLWidgets e Shiny di R. Il sistema è stato progettato con una architettura client-server: il client è responsabile della gestione dell'interfaccia, del rendering della network, della formulazione e della sottomissione delle query. Il server processa i dati dal database in base agli input dell'utente, e restituisce il risultato di tale computazione al client.
\end{frame}
...
Le distanze sono state calcolate per ogni coppia di nodi del grafo. Il numero di archi \e' quindi pari a $3686 * 3685 \approx 15 000 000$.
\end{alertblock}
...
Il pacchetto creato \'e installabile su qualsiasi sistema operativo avente R $\geq$ 3.2.
\noindent Per installarlo e' sufficiente eseguire i seguenti comandi da terminale:
\begin{lstlisting}[language=bash]
  $ git clone https://github.com/Abelarm/BioInf_Project
...
\noindent Per installare il tool Nanocluster e' sufficiente eseguire i seguenti comandi da terminale:
\begin{lstlisting}[language=bash]
  $ git clone https://github.com/Abelarm/BioInf_Project
